    ...
    const fetchContent = async () => {
      try {
        ...
        const filteredHeading1 = content.find(
          (item) => item.contentId === 62 && item.pageId === 1064 && item.sectionId === 2027 && item.controlId === 1
        );
        const filteredHeading2 = content.find(
          (item) => item.contentId === 63 && item.pageId === 1064 && item.sectionId === 2027 && item.controlId === 1
        );
        const filteredText1 = content.find(
          (item) => item.contentId === 64 && item.pageId === 1064 && item.sectionId === 2027 && item.controlId === 5
        );
        ...
        const filteredImages = content.filter(
          (item) => item.contentId === 34 || item.contentId === 35 || item.contentId === 36
        );
  ...
  const renderImage = (image) => {
    return (
      <Grid container spacing={2} justifyContent="center" paddingTop={'4%'}>
        {image && (
          <Grid item>
            <img src={`data:image/jpeg;base64,${image.contentData}`} alt={`content ${image.contentId}`} className="image-style" />
          </Grid>
        )}
      </Grid>
    );
      ...
      {loading ? (
        ...
        <>
          ...
          <Grid container spacing={2} className="content-grid">
            ...
            <Grid item xs={12}>
              <Typography variant="h4" className="buttons-button" style={{ textAlign: "center" }}>
                <Button
                  className={`custom-button ${activeButton === 1 ? 'active' : ''}`}
                  onClick={() => handleButtonClick(1)}
                >
                  CATERING PACKAGES
                </Button>
                <Button
                  className={`custom-button ${activeButton === 2 ? 'active' : ''}`}
