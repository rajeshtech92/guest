...
const Banquet = () => {
    ...
    useEffect(() => {
        const fetchContent = async () => {
            try {
                const response = await axios.get("https://guesthouse-api-dje8gvcwayfdfmbr.eastus-01.azurewebsites.net/api/Contents");
                const content = response.data;

                // Fetch images (if any) based on contentId 39
                const filteredImages = content.filter(image => image.contentId === 39);
                setImages(filteredImages);

                // Fetch other images based on specific contentIds
                const filteredImage = content.filter(image => [34, 35, 36].includes(image.contentId));
                setImage(filteredImage);

                // Fetch heading and text based on contentId, pageId, sectionId, and controlId
                const filteredHeading = content.find(item => item.contentId === 37 && item.pageId === 1064 && item.sectionId === 2019 && item.controlId === 1);
                const filteredText = content.find(item => item.contentId === 38 && item.pageId === 1064 && item.sectionId === 2019 && item.controlId === 5);
                const filteredText3 = content.find(item => item.contentId === 44 && item.pageId === 1064 && item.sectionId === 2022 && item.controlId === 5);
                const filteredHeading1 = content.find(item => item.contentId === 40 && item.pageId === 1064 && item.sectionId === 2021 && item.controlId === 1);
                const filteredText1 = content.find(item => item.contentId === 41 && item.pageId === 1064 && item.sectionId === 2021 && item.controlId === 5);
                const filteredHeading2 = content.find(item => item.contentId === 42 && item.pageId === 1064 && item.sectionId === 2021 && item.controlId === 1);
                const filteredText2 = content.find(item => item.contentId === 43 && item.pageId === 1064 && item.sectionId === 2021 && item.controlId === 5);
                ...
                <Grid container spacing={4} className="banquet-content">
                    ...
                    <Grid container spacing={0} className="banquet-content">
                        ...
                        <Grid item xs={12} onClick={() => onHome()}>
                            <Typography variant="h4" className="banquet-heading1" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
                                {heading1}
                            </Typography>
                        </Grid>
                        <Grid item xs={7}>
